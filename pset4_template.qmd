...
df_2016 = hospitals_2016[hospitals_2016['PGM_TRMNTN_CD'] == '00=ACTIVE PROVIDER']

# We create a function that helps us dind hospitals that were not active or disappeared by 2019
def check_closure(provider_num):
    # Check if the hospital is present in 2019 data
    hospital_2019 = hospitals_2019[hospitals_2019['PRVDR_NUM'] == provider_num]
    if hospital_2019.empty or hospital_2019['PGM_TRMNTN_CD'].iloc[0] != 'Active Provider':
        return True
    return False

# Apply the closure check and create a suspected closures DataFrame
suspected_closures = active_2016[active_2016['PRVDR_NUM'].apply(check_closure)]
suspected_closures['YEAR_CLOSED'] = 2019
print(f"Number of suspected hospital closures by 2019: {len(suspected_closures)}")

# Step 2: Sort suspected closures by facility name and report first 10 rows
suspected_closures_sorted = suspected_closures[['FAC_NAME', 'ZIP_CD', 'YEAR_CLOSED']].sort_values(by='FAC_NAME')
print(suspected_closures_sorted.head(10))

# Step 3: Filter out potential mergers
# Check if the number of active hospitals in a ZIP code decreased after suspected closure year
